    ...
    public static ArrayList<Menu> convertir(String[][] datos) {
        ...
        for (int i = 0; i < datos.length; i++) {
            for (int j = 0; j < datos[0].length; j++) {
                ...
                if (j == 1) {
                    val1 = Double.parseDouble(datos[i][j]);
                }
                if (j == 2) {
                    val2 = Double.parseDouble(datos[i][j]);
                }
                if (j == 3) {
                    val3 = Double.parseDouble(datos[i][j]);
                }
                if (j == 4) {
                    val4 = Double.parseDouble(datos[i][j]);
                }
            }
            if (datos[0].length == 3) {
                objeto1 = new MenuEconomico(nombre, val1, val2);
                objetos.add(objeto1);
            }
            if (datos[0].length == 4) {
                if (datos[0][0].equals("Niños 01")) {
                    objeto1 = new MenuNinios(nombre, val1, val2, val3);
                } else {
                    objeto1 = new MenuDia(nombre, val1, val2, val3);
                }

                objetos.add(objeto1);
            }
            if (datos[0].length == 5) {
                objeto1 = new MenuCarta(nombre, val1, val2, val3, val4);
                objetos.add(objeto1);
            }
        }
        return objetos;
